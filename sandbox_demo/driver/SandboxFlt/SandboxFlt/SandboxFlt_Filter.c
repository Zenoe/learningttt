...
#define RENAME_DIAG_SIZE 64
#define RENAME_DIAG_MASK (RENAME_DIAG_SIZE - 1)
...
typedef struct _RENAME_DIAG_ENTRY {
    ULONG Pid;
    ULONG SourceKey;
    ULONG DestKey;
    ULONG Class;
...
static RENAME_DIAG_ENTRY g_RenameDiag[RENAME_DIAG_SIZE];
static KSPIN_LOCK g_RenameDiagLock;
...
VOID Filter_InitRuntimeState(VOID)
{
    ...
    KeInitializeSpinLock(&g_RenameDiagLock);
}
...
static PFLT_INSTANCE Path_InstanceForOpen(
    _In_opt_ PFLT_INSTANCE Instance,
    _In_ PC_UNICODE_STRING Path)
{
    return Path_IsNtDevicePath(Path) ? NULL : Instance;
}

static NTSTATUS Path_BuildOnSameVolume(
    _In_ PC_UNICODE_STRING BasePath,
    _In_ PC_UNICODE_STRING RootRelativePath,
    _Out_ PUNICODE_STRING FullPath)
{
    ...
    if (!RootRelativePath->Buffer || RootRelativePath->Length == 0 ||
        RootRelativePath->Buffer[0] != L'\\') {
        return STATUS_INVALID_PARAMETER;
    }

    relOffset = 0;
    relLength = RootRelativePath->Length;
    if (RootRelativePath->Length >= 7 * sizeof(WCHAR) &&
        RootRelativePath->Buffer[0] == L'\\' &&
        RootRelativePath->Buffer[1] == L'?' &&
        RootRelativePath->Buffer[2] == L'?' &&
        RootRelativePath->Buffer[3] == L'\\' &&
        RootRelativePath->Buffer[5] == L':' &&
        RootRelativePath->Buffer[6] == L'\\') {
        relOffset = 6;
        relLength = RootRelativePath->Length - (relOffset * sizeof(WCHAR));
    }

    volumeEnd = Path_GetVolumeEnd(BasePath);
    if (volumeEnd == 0)
        return STATUS_INVALID_PARAMETER;

    totalChars = (USHORT)(volumeEnd +
        (relLength / sizeof(WCHAR)) + 1);
    buffer = (PWCHAR)ExAllocatePoolWithTag(NonPagedPool,
        totalChars * sizeof(WCHAR),
        SANDBOX_POOL_TAG);
    if (!buffer)
        return STATUS_INSUFFICIENT_RESOURCES;

    RtlCopyMemory(buffer, BasePath->Buffer, volumeEnd * sizeof(WCHAR));
    RtlCopyMemory(buffer + volumeEnd,
        RootRelativePath->Buffer + relOffset,
        relLength);
    buffer[totalChars - 1] = L'\0';

    FullPath->Buffer = buffer;
    FullPath->Length = (USHORT)((totalChars - 1) * sizeof(WCHAR));
    FullPath->MaximumLength = (USHORT)(totalChars * sizeof(WCHAR));
    return STATUS_SUCCESS;
}
...
static BOOLEAN
SetInfo_IsLink(_In_ FILE_INFORMATION_CLASS InfoClass)
{
    return (BOOLEAN)(InfoClass == FileLinkInformation ||
        InfoClass == FileLinkInformationEx);
}
...
static BOOLEAN
RenameDiag_StageEquals(
    _In_reads_(Size) const CHAR* Left,
    _In_z_ PCSTR Right,
    _In_ ULONG Size)
{
    ULONG i;
...
static VOID
RenameDiag_CopyStage(
    _Out_writes_(Size) CHAR* Dest,
    _In_ ULONG Size,
    _In_z_ PCSTR Source)
{
    ULONG i;
...
static VOID
SetInfo_LogRenameDiag(
    _In_z_ PCSTR Stage,
    _In_ ULONG Pid,
    _In_ FILE_INFORMATION_CLASS InfoClass,
    _In_ NTSTATUS Status,
    _In_ PC_UNICODE_STRING SourcePath,
    _In_opt_ PC_UNICODE_STRING DestPath)
{
    ULONG sourceKey;
    ULONG destKey;
    ULONG idx;
    ...
    BOOLEAN shouldLog;
    KIRQL oldIrql;
    RENAME_DIAG_ENTRY* entry;
    UNICODE_STRING emptyPath;
    PC_UNICODE_STRING printSource;
    PC_UNICODE_STRING printDest;

    sourceKey = SourcePath ? Hash_Unicode(SourcePath, 2166136261u) : 0;
    destKey = DestPath ? Hash_Unicode(DestPath, 2166136261u) : 0;
    RtlInitUnicodeString(&emptyPath, L"");
    printSource = SourcePath ? SourcePath : (PC_UNICODE_STRING)&emptyPath;
    printDest = DestPath ? DestPath : (PC_UNICODE_STRING)&emptyPath;
    idx = (sourceKey ^ destKey ^ Pid ^ (ULONG)InfoClass ^ (ULONG)Status) &
        RENAME_DIAG_MASK;
    shouldLog = FALSE;
    count = 1;

    KeAcquireSpinLock(&g_RenameDiagLock, &oldIrql);
    entry = NULL;
    for (i = 0; i < 4; ++i) {
        RENAME_DIAG_ENTRY* candidate = &g_RenameDiag[(idx + i) &
            RENAME_DIAG_MASK];
        if (candidate->Pid == Pid &&
            candidate->SourceKey == sourceKey &&
            candidate->DestKey == destKey &&
            candidate->Class == (ULONG)InfoClass &&
            candidate->Status == Status &&
            RenameDiag_StageEquals(candidate->Stage, Stage,
                sizeof(candidate->Stage))) {
            entry = candidate;
            break;
        }
        if (candidate->Pid == 0 && !entry)
            entry = candidate;
    }

    if (!entry)
        entry = &g_RenameDiag[idx];

    if (entry->Pid != Pid ||
        entry->SourceKey != sourceKey ||
        entry->DestKey != destKey ||
        entry->Class != (ULONG)InfoClass ||
        entry->Status != Status ||
        !RenameDiag_StageEquals(entry->Stage, Stage,
            sizeof(entry->Stage))) {
        ...
        entry->Class = (ULONG)InfoClass;
        entry->Status = Status;
        RenameDiag_CopyStage(entry->Stage, sizeof(entry->Stage), Stage);
        entry->Count = 1;
        count = 1;
        shouldLog = TRUE;
    }
        ...
        count = ++entry->Count;
        shouldLog = (BOOLEAN)((count & (count - 1)) == 0);
    }
    KeReleaseSpinLock(&g_RenameDiagLock, oldIrql);

    if (shouldLog) {
        DbgPrint("[SandboxFlt] RenameDiag stage=%s count=%lu pid=%lu class=%u status=%08x source=%wZ dest=%wZ\n",
            Stage,
            count,
            Pid,
            InfoClass,
...
static VOID
File_CloseKernelHandle(
    _In_opt_ HANDLE Handle,
    _In_opt_ PFILE_OBJECT FileObject,
    _In_ BOOLEAN OpenedWithZw)
{
    if (FileObject)
    ...
    _In_ PC_UNICODE_STRING TargetPath)
{
    ...
    BOOLEAN sourceOpenedWithZw;
    BOOLEAN targetOpenedWithZw;
    ...
    buffer = NULL;
    totalWritten = 0;
    sourceOpenedWithZw = FALSE;
    targetOpenedWithZw = FALSE;

    if (KeGetCurrentIrql() != PASSIVE_LEVEL)
        return STATUS_INVALID_DEVICE_STATE;

    sourceInstance = Path_InstanceForOpen(FltObjects->Instance, SourcePath);
    targetInstance = Path_InstanceForOpen(FltObjects->Instance, TargetPath);

    status = File_OpenKernelHandle(
        sourceInstance,
        SourcePath,
    ...
    RtlZeroMemory(&iosb, sizeof(iosb));
    status = ZwSetInformationFile(targetHandle,
        &iosb,
        &eofInfo,
        sizeof(eofInfo),
    ...
    for (;;) {
        bytesRead = 0;
        RtlZeroMemory(&iosb, sizeof(iosb));
        status = ZwReadFile(sourceHandle,
            NULL,
            NULL,
            NULL,
            &iosb,
        ...
        bytesWritten = 0;
        RtlZeroMemory(&iosb, sizeof(iosb));
        status = ZwWriteFile(targetHandle,
            NULL,
            NULL,
            NULL,
            &iosb,
...
Cleanup:
    ...
    File_CloseKernelHandle(targetHandle, targetFileObject, targetOpenedWithZw);
    File_CloseKernelHandle(sourceHandle, sourceFileObject, sourceOpenedWithZw);
    return status;
}
    ...
    _In_ PC_UNICODE_STRING SandboxTarget)
{
    ...
    PFLT_INSTANCE tempInstance;
    PFLT_INSTANCE targetInstance;
    BOOLEAN tempOpenedWithZw;
    BOOLEAN targetOpenedWithZw;
    ...
    buffer = NULL;
    totalWritten = 0;
    tempOpenedWithZw = FALSE;
    targetOpenedWithZw = FALSE;

    if (KeGetCurrentIrql() != PASSIVE_LEVEL)
        return STATUS_INVALID_DEVICE_STATE;

    tempInstance = Path_InstanceForOpen(FltObjects->Instance, SandboxTemp);
    targetInstance = Path_InstanceForOpen(FltObjects->Instance, SandboxTarget);
    ...
    RtlZeroMemory(&iosb, sizeof(iosb));
    closeStatus = ZwSetInformationFile(tempHandle,
        &iosb,
        &dispInfo,
        sizeof(dispInfo),
...
Cleanup:
    if (buffer)
        ExFreePoolWithTag(buffer, SANDBOX_POOL_TAG);
    File_CloseKernelHandle(targetHandle, targetFileObject, targetOpenedWithZw);
    File_CloseKernelHandle(tempHandle, tempFileObject, tempOpenedWithZw);
    return status;
}
    ...
    _In_ PC_UNICODE_STRING TempPath)
{
    ...
    tempInstance = Path_InstanceForOpen(FltObjects->Instance,
    ...
    RtlZeroMemory(&iosb, sizeof(iosb));
    status = ZwSetInformationFile(tempHandle,
        &iosb,
        renameInfo,
        renameLength,
    ...
    _Outptr_result_maybenull_ PVOID* AllocatedInfo)
{
    ...
    *AllocatedInfo = NULL;
    ...
    DbgPrint("[SandboxFlt] RenameTarget enter class=%u source=%wZ dest=%wZ rootDir=%p len=%lu flags=%08x\n",
    ...
    status = Path_BuildRedirect((PC_UNICODE_STRING)&destInfo->Name,
        Box, &sandboxDest);
    if (!NT_SUCCESS(status)) {
        DbgPrint("[SandboxFlt] RenameTarget build dest redirect failed: %08x dest=%wZ boxRoot=%wZ\n",
            status,
            &destInfo->Name,
            &Box->SandboxRootNt);
        goto Cleanup;
    ...
    DbgPrint("[SandboxFlt] RenameTarget redirecting class=%u source=%wZ dest=%wZ sandboxDest=%wZ\n",
        Data->Iopb->Parameters.SetFileInformation.FileInformationClass,
        SourcePath,
        &destInfo->Name,
        &sandboxDest);
...
Cleanup:
    if (!NT_SUCCESS(status)) {
        DbgPrint("[SandboxFlt] RenameTarget exit failed: %08x source=%wZ sandboxDest=%wZ\n",
            status,
            SourcePath,
            &sandboxDest);
    }
...
SetInfo_RenameSandboxSourceToRedirectTarget(
    ...
    _In_ ULONG Pid,
    _In_ PC_UNICODE_STRING SourcePath,
    _In_ BOOLEAN SourceAlreadyInSandbox)
{
    PFILE_RENAME_INFORMATION oldInfo;
    PFILE_RENAME_INFORMATION renameInfo;
    PFLT_FILE_NAME_INFORMATION destInfo;
    PC_UNICODE_STRING destPath;
    UNICODE_STRING rawDest;
    UNICODE_STRING normalizedRawDest;
    UNICODE_STRING relDest;
    ...
    NTSTATUS status;
    PFLT_INSTANCE sourceInstance;
    BOOLEAN sandboxSourceAllocated;
    BOOLEAN sandboxDestAllocated;
    BOOLEAN normalizedRawDestAllocated;
    BOOLEAN destInSandbox;
    BOOLEAN sourceOpenedWithZw;
    BOOLEAN isLinkInfo;
    BOOLEAN isWordLinkTemp;
    BOOLEAN usedCurrentFileObject;
    ...
    sourceFileObject = NULL;
    renameInfo = NULL;
    sandboxSourceAllocated = FALSE;
    sandboxDestAllocated = FALSE;
    normalizedRawDestAllocated = FALSE;
    destInSandbox = FALSE;
    sourceOpenedWithZw = FALSE;
    isLinkInfo = SetInfo_IsLink(
        Data->Iopb->Parameters.SetFileInformation.FileInformationClass);
    isWordLinkTemp = FALSE;
    usedCurrentFileObject = FALSE;
    RtlZeroMemory(&sandboxSource, sizeof(sandboxSource));
    RtlZeroMemory(&sandboxDest, sizeof(sandboxDest));
    RtlZeroMemory(&rawDest, sizeof(rawDest));
    RtlZeroMemory(&normalizedRawDest, sizeof(normalizedRawDest));
    ...
    if (!NT_SUCCESS(status)) {
        ...
        rawDest.Buffer = oldInfo->FileName;
        rawDest.Length = (USHORT)oldInfo->FileNameLength;
        rawDest.MaximumLength = rawDest.Length;
        if (Path_IsNtDevicePath((PC_UNICODE_STRING)&rawDest)) {
            destPath = (PC_UNICODE_STRING)&rawDest;
        }
        else {
            status = Path_BuildOnSameVolume(SourcePath,
                (PC_UNICODE_STRING)&rawDest,
                &normalizedRawDest);
            if (!NT_SUCCESS(status)) {
                DbgPrint("[SandboxFlt] RenameSandboxSource normalize raw dest failed: %08x source=%wZ rawDest=%wZ\n",
                    status,
                    SourcePath,
                    &rawDest);
                goto Cleanup;
    ...
    if (isLinkInfo && isWordLinkTemp) {
        SetInfo_LogRenameDiag("skip-word-link",
            Pid,
            Data->Iopb->Parameters.SetFileInformation.FileInformationClass,
            STATUS_SUCCESS,
            SourcePath,
    ...
    destInSandbox = Path_IsInSandbox(destPath, Box);

    if (!destInSandbox && Path_IsExcluded((PC_UNICODE_STRING)&relDest,
        (PC_UNICODE_STRING)&Box->SandboxRootNt)) {
        status = STATUS_ACCESS_DENIED;
        goto Cleanup;
    }
    ...
    if (SourceAlreadyInSandbox) {
        sandboxSource = *SourcePath;
    }
    else {
        status = Path_BuildRedirect(SourcePath, Box, &sandboxSource);
        if (!NT_SUCCESS(status)) {
            DbgPrint("[SandboxFlt] RenameSandboxSource build source redirect failed: %08x source=%wZ boxRoot=%wZ\n",
                status,
                SourcePath,
                &Box->SandboxRootNt);
            goto Cleanup;
        }
        sandboxSourceAllocated = TRUE;
    }

    sourceInstance = Path_InstanceForOpen(FltObjects->Instance,
        (PC_UNICODE_STRING)&sandboxSource);

    if (destInSandbox) {
        sandboxDest = *destPath;
    }
    else {
        status = Path_BuildRedirect(destPath,
            Box,
            &sandboxDest);
        if (!NT_SUCCESS(status)) {
            DbgPrint("[SandboxFlt] RenameSandboxSource build dest redirect failed: %08x dest=%wZ boxRoot=%wZ\n",
                status,
                destPath,
                &Box->SandboxRootNt);
            goto Cleanup;
    ...
    if (SourceAlreadyInSandbox) {
        ...
        status = FltSetInformationFile(FltObjects->Instance,
            ...
            Data->Iopb->Parameters.SetFileInformation.FileInformationClass);
        if (!NT_SUCCESS(status)) {
            SetInfo_LogRenameDiag("set-current-failed",
                Pid,
                Data->Iopb->Parameters.SetFileInformation.FileInformationClass,
                status,
                (PC_UNICODE_STRING)&sandboxSource,
    ...
    if (!NT_SUCCESS(status)) {
        SetInfo_LogRenameDiag("open-source",
            Pid,
            Data->Iopb->Parameters.SetFileInformation.FileInformationClass,
            status,
            (PC_UNICODE_STRING)&sandboxSource,
    ...
    if (status == STATUS_OBJECT_NAME_NOT_FOUND ||
        ...
        (SourceAlreadyInSandbox &&
            SetInfo_ShouldCopyFallbackRename(status))) {
        if (SourceAlreadyInSandbox) {
            if (!isLinkInfo && SetInfo_ShouldCopyFallbackRename(status)) {
                NTSTATUS copyStatus;

                SetInfo_LogRenameDiag("open-copy-fallback",
                    Pid,
                    Data->Iopb->Parameters.SetFileInformation.FileInformationClass,
                    status,
                    (PC_UNICODE_STRING)&sandboxSource,
                    (PC_UNICODE_STRING)&sandboxDest);
                copyStatus = File_CopyContentsNoDelete(FltObjects,
                    (PC_UNICODE_STRING)&sandboxSource,
                    (PC_UNICODE_STRING)&sandboxDest);
                if (NT_SUCCESS(copyStatus)) {
                    ...
                    Cache_Add(g_WritePathCache, WRITE_PATH_CACHE_SIZE,
                        writeKey);
                    SandboxFlt_RecordRedirectPath(
                        (PC_UNICODE_STRING)&sandboxDest);
                    InterlockedIncrement(&g_Sandbox.TotalRedirects);
                    SetInfo_LogRenameDiag("open-copy-ok",
                        Pid,
                        Data->Iopb->Parameters.SetFileInformation.FileInformationClass,
                        copyStatus,
                        (PC_UNICODE_STRING)&sandboxSource,
                ...
                SetInfo_LogRenameDiag("open-copy-failed",
                    Pid,
                    Data->Iopb->Parameters.SetFileInformation.FileInformationClass,
                    copyStatus,
                    (PC_UNICODE_STRING)&sandboxSource,
            ...
            if (!isLinkInfo && Path_SandboxFileExists(FltObjects,
                Box,
                (PC_UNICODE_STRING)&sandboxDest)) {
                writeKey = Path_WriteCacheKey(destPath, Box);
                Cache_Add(g_WritePathCache, WRITE_PATH_CACHE_SIZE, writeKey);
                SandboxFlt_RecordRedirectPath((PC_UNICODE_STRING)&sandboxDest);
                InterlockedIncrement(&g_Sandbox.TotalRedirects);
                SetInfo_LogRenameDiag("already-renamed",
                    Pid,
                    Data->Iopb->Parameters.SetFileInformation.FileInformationClass,
                    status,
                    (PC_UNICODE_STRING)&sandboxSource,
        ...
        status = File_CopyContentsNoDelete(FltObjects,
            ...
            (PC_UNICODE_STRING)&sandboxSource);
        if (!NT_SUCCESS(status)) {
            DbgPrint("[SandboxFlt] RenameSandboxSource migrate failed: %08x source=%wZ sandboxSource=%wZ\n",
                status,
                SourcePath,
                &sandboxSource);
            goto Cleanup;
        ...
        File_CloseKernelHandle(sourceHandle, sourceFileObject,
            sourceOpenedWithZw);
        sourceHandle = NULL;
        sourceFileObject = NULL;
        sourceOpenedWithZw = FALSE;
...
HandleSetInfoResult:
    if (NT_SUCCESS(status)) {
        ...
        SetInfo_ShouldCopyFallbackRename(status) &&
        !(usedCurrentFileObject && status == STATUS_VHD_INVALID_STATE)) {
        SetInfo_LogRenameDiag("rename-fallback",
            Pid,
            Data->Iopb->Parameters.SetFileInformation.FileInformationClass,
            status,
            (PC_UNICODE_STRING)&sandboxSource,
        ...
        if (NT_SUCCESS(status)) {
            ...
            SetInfo_LogRenameDiag("copy-failed",
                Pid,
                Data->Iopb->Parameters.SetFileInformation.FileInformationClass,
                status,
                (PC_UNICODE_STRING)&sandboxSource,
            ...
            if (Path_SandboxFileExists(FltObjects,
                ...
                SandboxFlt_RecordRedirectPath((PC_UNICODE_STRING)&sandboxDest);
                InterlockedIncrement(&g_Sandbox.TotalRedirects);
                SetInfo_LogRenameDiag("copy-dest-exists",
                    Pid,
                    Data->Iopb->Parameters.SetFileInformation.FileInformationClass,
                    status,
                    (PC_UNICODE_STRING)&sandboxSource,
...
Cleanup:
    if (!NT_SUCCESS(status)) {
        DbgPrint("[SandboxFlt] RenameSandboxSource exit failed: %08x source=%wZ sandboxSource=%wZ sandboxDest=%wZ\n",
            status,
            SourcePath,
            &sandboxSource,
            &sandboxDest);
    ...
    File_CloseKernelHandle(sourceHandle, sourceFileObject,
        sourceOpenedWithZw);
    if (sandboxSourceAllocated && sandboxSource.Buffer)
        ExFreePoolWithTag(sandboxSource.Buffer, SANDBOX_POOL_TAG);
    if (sandboxDestAllocated && sandboxDest.Buffer)
        ExFreePoolWithTag(sandboxDest.Buffer, SANDBOX_POOL_TAG);
    if (normalizedRawDestAllocated && normalizedRawDest.Buffer)
        ExFreePoolWithTag(normalizedRawDest.Buffer, SANDBOX_POOL_TAG);
    if (destInfo)
        FltReleaseFileNameInformation(destInfo);
    return status;
}
    ...
    if (SetInfo_IsRenameOrLink(infoClass)) {
        ...
        if (sourceInSandbox) {
            if (SetInfo_ShouldCopyFallbackRename(status)) {
                SetInfo_LogRenameDiag("noop-after-fail",
                    pid,
                    infoClass,
                    status,
                    (PC_UNICODE_STRING)&sourceInfo->Name,
    ...
    _Outptr_ PDIR_MERGE_CONTEXT* OutCtx)
{
    ...
    *OutCtx = NULL;
    ...
    InitializeObjectAttributes(&oa, &sandboxPath,
        OBJ_KERNEL_HANDLE | OBJ_CASE_INSENSITIVE, NULL, NULL);
    openInstance = Path_InstanceForOpen(FltObjects->Instance,
        (PC_UNICODE_STRING)&sandboxPath);
